...
### Calculate accuracy by REPU
  Summarize the results from the 100% simulation, calculate % unassigned, write out
```{r}
summarize_indv_assignment = as.data.frame(matrix(NA,
                                                 nrow = length(unique(hundy_sim_summary_mean_pi_mle$repunit)),
                                                 ncol = length(unique(hundy_sim_summary_mean_pi_mle$repunit_scenario))))
j=1
i=1
i=NULL
for (i in 1:length(unique(hundy_sim_summary_mean_pi_mle$repunit_scenario))) {
  for (j in 1:length(unique(hundy_sim_summary_mean_pi_mle$repunit))) {
    x = which(hundy_sim_summary_mean_pi_mle$repunit_scenario == unique(hundy_sim_summary_mean_pi_mle$repunit_scenario)[i])
    summarize_indv_assignment[j,i] = hundy_sim_summary_mean_pi_mle[x[j],3]
  }
}

colnames(summarize_indv_assignment) = unique(hundy_sim_summary_mean_pi_mle$repunit_scenario)
rownames(summarize_indv_assignment) = unique(hundy_sim_summary_mean_pi_mle$repunit)
#see dev/GSI_pipe for section designed to calculate the proportion unassigned 

# write.csv(summarize_indv_assignment,paste("./results/Panel_tests/",panel_name,"/","100%_SIM-",panel_name,".csv",sep = ""))
```

### Format results for plot
```{r}
#capture results in test data frame
test_summarize_indv_assignment = summarize_indv_assignment
#make the rownames (the lake assigned to) a variable (FACTOR)
test_summarize_indv_assignment$assigned_to = factor(rownames(test_summarize_indv_assignment))
#Melt
summarize_indv_assignment_melted = reshape2::melt(test_summarize_indv_assignment)
#rename columns
names(summarize_indv_assignment_melted) = c("assigned_to","simulated_from","proportion")
#remove useless strings
summarize_indv_assignment_melted$assigned_to = gsub("Rep_U-","",summarize_indv_assignment_melted$assigned_to)
summarize_indv_assignment_melted$simulated_from = gsub("All-Rep_U-","",summarize_indv_assignment_melted$simulated_from)
#turn 0's into NA values
summarize_indv_assignment_melted[which(summarize_indv_assignment_melted$proportion < 0.005),3] = NA
#reorder levels to mach table order
# summarize_indv_assignment_melted$assigned_to = factor(summarize_indv_assignment_melted$assigned_to, levels = "")

auto_axis_list = NULL
for (i in 1:n_distinct(summarize_indv_assignment_melted$assigned_to)) {
  auto_axis_list = paste(auto_axis_list, "'",unique(summarize_indv_assignment_melted$assigned_to)[i],"'",",", sep = "")
}
auto_axis_list

#Block that sets x and y values according to  your hardcode
#First collect the names of your simulation scenarios
edit_axis = as.data.frame(unique(summarize_indv_assignment_melted$simulated_from))
#Second, add a numeric value that defines BOTH x and Y coordinate (same as we want self assignment on the diagonal)
edit_axis$repU_plot_order = c(4,10,1,7,8,11,13,9,12,6,5,3,2)
names(edit_axis) = c("Pop_ID","Plot_order")
#loop adds the x y data to the data frame with proportions to plot
summarize_indv_assignment_melted$simed_from_Ycoord = NA
summarize_indv_assignment_melted$assigned_to_Xcoord = NA
for (i in 1:nrow(edit_axis)) {
  summarize_indv_assignment_melted[which(summarize_indv_assignment_melted$simulated_from == edit_axis[i,1]),"simed_from_Ycoord"] = edit_axis[i,2]
  summarize_indv_assignment_melted[which(summarize_indv_assignment_melted$assigned_to == edit_axis[i,1]),"assigned_to_Xcoord"] = edit_axis[i,2]
}
#organize so we have ascending assignments within ascending simulation scenarios
summarize_indv_assignment_melted = summarize_indv_assignment_melted %>% arrange(assigned_to_Xcoord)
summarize_indv_assignment_melted = summarize_indv_assignment_melted %>% arrange(simed_from_Ycoord)
rownames(summarize_indv_assignment_melted) = 1:nrow(summarize_indv_assignment_melted)

save(summarize_indv_assignment_melted, file = paste("./simulation_result_", panel_name,"byREPU.rda", sep = ""))
...
ggplot(summarize_indv_assignment_melted, aes(x = assigned_to_Xcoord, y = simed_from_Ycoord, size = proportion))+
  geom_point(shape = 21, fill = "gray", color = "black", 
             alpha = 0.5, inherit.aes = T, 
             size = (18*summarize_indv_assignment_melted$proportion))+
...
ggsave(paste("./100_Sims_",panel_name,".jpg"), width = 10, height = 10, dpi = 350)
# write.csv(summarize_indv_assignment_melted,"./EDIT.csv")
```
...
### Calculate accuracy by POP

test = repu_hundy_results$mixing_proportions

test_collect = rep(test[1:length(which(!is.na(test$repunit))),"collection"], times = n_distinct(test$repunit_scenario))
test$collection = test_collect$collection

test[999:1010,]
#calculate the net simulated mle_pi for each reporting unit, by iteration, for each 100% collection
hundy_sim_summary = test %>% 
  group_by(collection_scenario, iter, collection) %>% 
  summarise(net_pi_mle_by_pop = round(sum(mle_pi),2)) %>% 
  ungroup()
#calculate an average simulated mle_pi for each reporting unit, over all iterations, for each 100% collection
hundy_sim_summary_mean_pi_mle = hundy_sim_summary %>% 
  group_by(collection_scenario, collection) %>% 
  summarise(avg_pi_mle_by_pop = mean(net_pi_mle_by_pop))
#calculate 95%CI around the mean
hundy_sim_summary_95CI = hundy_sim_summary %>% 
  group_by(collection_scenario, collection) %>% 
  summarise(CI95_pi_mle_by_pop = (1.96*(sd(net_pi_mle_by_pop)/sqrt(length(net_pi_mle_by_pop)))))


summarize_indv_assignment = as.data.frame(matrix(NA,
                                                 nrow = length(unique(hundy_sim_summary_mean_pi_mle$collection)),
                                                 ncol = length(unique(hundy_sim_summary_mean_pi_mle$collection_scenario))))
j=1
i=1
i=NULL
for (i in 1:length(unique(hundy_sim_summary_mean_pi_mle$collection_scenario))) {
  for (j in 1:length(unique(hundy_sim_summary_mean_pi_mle$collection))) {
    x = which(hundy_sim_summary_mean_pi_mle$collection_scenario == unique(hundy_sim_summary_mean_pi_mle$collection_scenario)[i])
    summarize_indv_assignment[j,i] = hundy_sim_summary_mean_pi_mle[x[j],3]
  }
}

colnames(summarize_indv_assignment) = unique(hundy_sim_summary_mean_pi_mle$collection_scenario)
rownames(summarize_indv_assignment) = unique(hundy_sim_summary_mean_pi_mle$collection)
#see dev/GSI_pipe for section designed to calculate the proportion unassigned 
```
...
```
### Format results for plot
 hardcode assigned order by assigning the xy position you want to each name as it appears in "auto_axis_list"
```{r}
#Block that sets x and y values according to  your hardcode
#First collect the names of your simulation scenarios
edit_axis = as.data.frame(unique(summarize_indv_assignment_melted$simulated_from))
edit_axis$assigned_to = unique(summarize_indv_assignment_melted$assigned_to)
#Second, add a numeric value that defines BOTH x and Y coordinate (same as we want self assignment on the diagonal)
edit_axis$assigned_to_order = c(7, 10, 17, 1, 11, 6, 14, 3, 8, 4, 15, 18, 20, 16, 19, 13, 12, 9, 5, 2)
names(edit_axis) = c("simulated_from","assigned_to","assigned_to_order")

edit_axis$simulated_from_order = rownames(edit_axis)


# panel_name = "FST_600"
# panel_name = "mhapHE_600"
# panel_name = "Composite_600"
# load(file = paste("./simulation_result_",panel_name,"_byPOP.rda", sep = ""))
#loop adds the x y data to the data frame with proportions to plot
summarize_indv_assignment_melted$simed_from_Ycoord = NA
summarize_indv_assignment_melted$assigned_to_Xcoord = NA

for (i in 1:nrow(edit_axis)) {
  summarize_indv_assignment_melted[which(summarize_indv_assignment_melted$simulated_from == edit_axis[i,1]),"simed_from_Ycoord"] = edit_axis[i,3]
  
  summarize_indv_assignment_melted[which(summarize_indv_assignment_melted$assigned_to == edit_axis[i,2]),"assigned_to_Xcoord"] = edit_axis[i,3]
}
#organize so we have ascending assignments within ascending simulation scenarios
summarize_indv_assignment_melted = summarize_indv_assignment_melted %>% arrange(assigned_to_Xcoord)
summarize_indv_assignment_melted = summarize_indv_assignment_melted %>% arrange(simed_from_Ycoord)
...
save(summarize_indv_assignment_melted, file = paste("./simulation_result_",panel_name,"_byPOP.rda", sep = ""))
...
ggsave(paste("./100_Sims_", panel_name, ".jpg", sep = ""), width = 20, height = 20, dpi = 350)
# write.csv(summarize_indv_assignment_melted,"./EDIT.csv")
# 
beep()
Sys.sleep(2)
