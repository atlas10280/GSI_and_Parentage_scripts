...
# write final table
write.table(summary_table_final, "./TEST_data/test_hap_summary_table.tsv", quote = F, row.names = F, sep = "\t")
```
...
```{r}
bstats = basicStats("../../Populations/v4H_global_populations.snps.gen","v4H_global_Fis.txt",
...
#check Fst using diveRsity (diffcalc)
```{r}
...
Fstats = diffCalc("../../Populations/Haploid/populations.haplotypes.gen","./Fst_haps-diveRsity", fst = TRUE, pairwise = FALSE, bs_locus = FALSE)
```
...
#haplotype Ho and He
```{r}
snps = read.genepop("../../Populations/Haploid/populations.haplotypes.gen", ncode = 2)
synopsis_snps = summary(snps)
